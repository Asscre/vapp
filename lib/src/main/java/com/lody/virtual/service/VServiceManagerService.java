 ...
 * 提供服务管理相关的Binder服务
 */
public class VServiceManagerService implements IBinder, IInterface {
    
    private static final String TAG = "VServiceManagerService";
    
    private Context mContext;
    
    public VServiceManagerService(Context context) {
        mContext = context.getApplicationContext();
    }
    
    @Override
    public String getInterfaceDescriptor() {
        return "com.lody.virtual.service.IServiceManager";
    }
    ...
    public android.os.IInterface queryLocalInterface(String descriptor) {
        if ("com.lody.virtual.service.IServiceManager".equals(descriptor)) {
            return this;
        }
        return null;
    }
    ...
    public boolean transact(int code, android.os.Parcel data, android.os.Parcel reply, int flags) {
        try {
            ...
            switch (code) {
                ...
                case 2: // UNBIND_SERVICE
                    return handleUnbindService(data, reply);
    ...
    public IBinder bindService(Intent intent) {
        try {
            Log.d(TAG, "Binding virtual service: " + intent);
    ...
    private boolean handleBindService(android.os.Parcel data, android.os.Parcel reply) {
        try {
            ...
            IBinder result = bindService(intent);
     ...
     * 处理解绑服务请求
     */
    private boolean handleUnbindService(android.os.Parcel data, android.os.Parcel reply) {
        try {
            // 读取Intent数据
            Intent intent = Intent.CREATOR.createFromParcel(data);
            
            Log.d(TAG, "Handling unbind service for: " + intent);
            ...
            Log.e(TAG, "Exception in handleUnbindService", e);
            return false;
        }
    }
